 ...
 */
function getAuthorization(settings, logger) {
  return new q.Promise((resolve, reject) => {
    try {
      // try and get the token from the cache
      const token = tokenCache.get('token', true);
      logger.info('Used old token.');
      resolve(`Token ${token}`);
    } catch (err) {
      // apperently the key was not found
      getFlarumToken(settings, logger).then((token) => {
